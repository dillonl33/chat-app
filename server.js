...
// end
  ...
  socket.on('joinRoom', ({ username, room }) => {
    ...
    // Load previous messages. this should only find history for user to user, but this is not specified here, as I don't know how to send that. maybe i could add a boolean variable if necessary.
      ...
      var temp2;
      var temp3 = [];
      client.query('SELECT senderid, message, time from chats where senderid = (select uid from users where username = \'' + user.username + '\') and receiverid = (select uid from users where username = \'' + user.room + '\');', (err, ret) => {
